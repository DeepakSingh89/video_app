...
import '../../view/home/home_page/home_module/get_all_catagary_model.dart';

class CatagaryProvider {
  late Dio client;

  CatagaryProvider({required this.client});

  catagaryApi() async {
    try {
      Response r = await client.get('api/get/category');
      if (r.statusMessage == 'OK') {
        //prefs.write('token', r.data['token']);

        print(token);
        return getCatagaryModelFromJson(jsonEncode(r.data));
      } else {
