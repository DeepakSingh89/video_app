...
class CupponProvider {
  late Dio client;

  CupponProvider({required this.client});

  getcupponApi() async {
    try {
      Response r = await client.get('api/get/coupon');
      if (r.statusMessage == 'OK') {
        //prefs.write('token', r.data['token']);

        print(token);
        return getCupponModelFromJson(jsonEncode(r.data['getCoupon']));
      } else {
